...
"""
merge_diar_any.py
Склейка транскрипта (SRT/JSON от whisper.cpp или OpenAI) и RTTM (pyannote) → .spk.srt/.spk.txt/.spk.json

Особенности:
- Устойчивый парсер сегментов из самых разных JSON-схем (segments / transcription / items / words[] / timestamps / offsets).
- Выбор спикера по максимальному перекрытию сегмента с окнами из RTTM (fallback — ближайший по центру).
- Поиск RTTM с толерантностью к переименованиям: пробелы → "_"/удалены/"%20", сравнение stem без пробелов/подчёркиваний.
"""
...
def _join_words(words):
    ...
    # Иногда слова уже содержат ведущие пробелы — склейка без доп. пробела корректна
    txt = "".join(parts).strip()
    # Если вообще нет пробелов (редкие кейсы), поставим защитный пробел
    if txt and " " not in txt and len(parts) > 1:
...
def _segments_from_list(lst):
    ...
    Возвращает None, если список не похож на массив сегментов.
    """
    out = []
    any_have_time = False
    ...
    for it in lst:
        ...
        # 4) timestamps: {from:"HH:MM:SS,mmm", to:"..."}  ← частый кейс whisper.cpp JSON
...
def choose_spk_overlap(rttm_segs, seg_start, seg_end):
    """Находит спикера с максимальным перекрытием с сегментом ASR. Если перекрытия нет — None."""
    best = None
    best_overlap = 0.0
    for s, e, spk in rttm_segs:
        # пересечение отрезков [s,e] и [seg_start, seg_end]
        left = max(s, seg_start)
        right = min(e, seg_end)
...
def choose_spk_midpoint(rttm_segs, tmid):
    """Fallback: возвращает спикера, покрывающего середину сегмента, или ближайшего по центру окна."""
    for s, e, spk in rttm_segs:
        if s <= tmid <= e:
            return spk
    if rttm_segs:
...
def norm_spk_tag(spk):
    ...
    return f"SPK{int(m.group(1)):02d}" if m else (spk or "SPK??").upper()


# ---------- ПОИСК RTTM С ТОЛЕРАНТНОСТЬЮ ----------


def _variants_without_spaces(name: str):
    """Варианты имён для соответствия: как есть / с '_' / без пробелов / c %20."""
    return {
        name,
        name.replace(" ", "_"),
        name.replace(" ", ""),
...
def _normalize_stem_for_match(s: str) -> str:
    """Нормализуем для сравнения: нижний регистр, удаляем пробелы и '_'."""
    return re.sub(r"[_\s]+", "", s or "").lower()


def resolve_rttm_path(rttm_root: Path, base_rel: Path) -> Path | None:
    """
    Пытается найти RTTM:
    1) Точный путь rttm_root/base_rel.with_suffix('.rttm')
    2) Варианты: пробелы → '_'/удалить/'%20' по каждому компоненту
    3) Если папка есть — ищем .rttm со схожим stem в папке (сравнение stem без пробелов/подчёркиваний)
    """
    # 1) точный путь
    exact = (rttm_root / base_rel).with_suffix(".rttm")
    if exact.exists():
        return exact

    # 2) варианты по компонентам пути
    parts = list(base_rel.parts)
    candidates = set()

    # пробегаем по комбинациям замен в каждом компоненте
    def generate_paths(idx: int, acc: list[str]):
        if idx == len(parts):
            candidates.add(rttm_root.joinpath(*acc).with_suffix(".rttm"))
            return
        for v in _variants_without_spaces(parts[idx]):
            generate_paths(idx + 1, acc + [v])
    ...
    # 3) поиск по папке: совпадение stem-нормализации
    parent = rttm_root / base_rel.parent
    if parent.exists():
        want = _normalize_stem_for_match(base_rel.stem)
        for rp in parent.glob("*.rttm"):
            if _normalize_stem_for_match(rp.stem) == want:
                return rp
...
def process_one(tr_path: Path, base_rel: Path, rttm_root: Path, out_root: Path):
    ...
    # подобрать rttm по относительному пути с толерантностью
    rttm_path = resolve_rttm_path(rttm_root, base_rel)
    if not rttm_path:
        # если расширение не .json/.srt, попробуем без него
        rttm_path = resolve_rttm_path(rttm_root, base_rel.with_suffix(""))
    rttm = load_rttm(rttm_path) if rttm_path else []
    ...
    # нормализуем нулевые/инвертированные тайминги
    cleaned = []
    for seg in segs:
        ...
        cleaned.append({"start": st, "end": en, "text": txt})
    segs = cleaned

    # назначить спикеров (сначала по перекрытию, потом fallback по середине)
    out_segs = []
    for seg in segs:
        s0, s1 = seg["start"], seg["end"]
        spk = choose_spk_overlap(rttm, s0, s1) or choose_spk_midpoint(
            rttm, (s0 + s1) / 2.0
        )
        spk = norm_spk_tag(spk)
        out_segs.append(
    ...
    # диагнóstика
    if not rttm_path:
        return True, f"{str(out_base)} (RTTM not found; assigned by nearest)"
    if not rttm:
        return True, f"{str(out_base)} (empty RTTM; assigned by nearest)"
    return True, str(out_base)


def main():
    ap = argparse.ArgumentParser(
        description="Склейка транскрипта (SRT/JSON от whisper.cpp/OpenAI) и RTTM (pyannote) → .spk.srt/.spk.txt/.spk.json"
    )
    ap.add_argument(
        "--transcripts",
        required=True,
