...
def ascii_slug(text: str, salt: str) -> str:
    """
    Делает ASCII-имя + всегда добавляет короткий хэш от 'salt' (обычно относительный путь).
    Это гарантирует уникальность и убирает коллизии типа 'audio.m4a'.
    """
    s = unicodedata.normalize("NFKD", text).encode("ascii", "ignore").decode("ascii")
    s = re.sub(r"[^A-Za-z0-9._-]+", "_", s).strip("_")
    h = hashlib.md5(salt.encode("utf-8")).hexdigest()[:8]
    base = s or "file"
    # ограничим длину базового слага, чтобы путь не раздувать
    if len(base) > 64:
...
@dataclass
class PartInfo:
    path: str  # путь к части (относительно dst_root)
    duration: float
    offset: float
...
def encode_audio_primary(
    ...
    cmd = [
        ...
        # иногда mp4 с повреждённым индексом читается лучше с увеличенным анализом
        "-analyzeduration",
        "200M",
        "-probesize",
        "200M",
...
def encode_audio_fallback(
    ...
    """
    Фоллбэк для реально битых контейнеров: попробуем принудительный декод и повторную сборку.
    """
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if output_path.exists() and not overwrite:
        return True, "skip"
    ...
    cmd = [
        ...
        "-nostdin",
        "-y" if overwrite else "-n",
        # агрессивные опции для странных контейнеров/таймстампов
        "-analyzeduration",
        "400M",
        "-probesize",
        "400M",
...
def rel_output_path(
    src_dir: Path, file: Path, dst_dir: Path, ascii_names: bool
) -> Path:
    rel = file.relative_to(src_dir)
    if ascii_names:
        # используем относительный путь как соль, чтобы имя было стабильным и уникальным
        salt = str(rel)
...
def task_convert_one(
    ...
    try:
        out_m4a = rel_output_path(src_dir, inp, dst_dir, ascii_names)
        print(f"→ SRC: {inp}\n  DST: {out_m4a}")
        ...
        # иногда при провале кодека остаётся нулевой/битый файл
        if actual_size == 0:
        ...
        try:
            ...
            # если ffprobe на выходе тоже упал — считаем файл битым
            return ("err", inp, f"ffprobe on output failed:\n{e}")

        base_noext = out_m4a.with_suffix("")
        if actual_size <= size_limit_bytes:
        ...
        # удаляем крупный исходный m4a (он нам уже не нужен)
        try:
            out_m4a.unlink()
        except Exception:
            pass

        return ("ok", inp, f"{len(parts)} parts")
...
def main():
    ap = argparse.ArgumentParser(
        description="Extract audio → m4a (AAC 96k, 16kHz mono). If > size-limit (24MB) — split to parts and write manifest."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        type=float,
        default=24.0,
        help="Лимит размера для Whisper API (MB)",
    )
    ap.add_argument(
        "--ascii-names",
        action="store_true",
        help="ASCII-имена выходных файлов + хэш (рекомендую включать)",
    )
    args = ap.parse_args()
    ...
    print(
        f"[INFO] Found {len(inputs)} files → m4a @ {args.sr} Hz, {args.channels} ch, aac {args.bitrate}, size-limit={args.size_limit_mb} MB"
    )
